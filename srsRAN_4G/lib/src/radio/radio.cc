...
int radio::init(const rf_args_t& args, phy_interface_radio* phy_)
{
  ...
  // Testing switch for STEEP
  steep_enabled_ = true;
  // printf(">>> STEEP IS ENABLED <<<\n");
  // fflush(stdout);
...
bool radio::tx(rf_buffer_interface& buffer, const rf_timestamp_interface& tx_time)
{
  ...
  if (ratio > 1 && (size_t)nof_samples * (size_t)ratio > tx_buffer[0].size()) {
    // ... bounds-check block unchanged ...
    nof_samples = tx_buffer[0].size() / ratio;
  }

  // === STEEP TX hook - BEFORE the resampler ===
  if (steep_enabled_) {
    const srsran_timestamp_t ts = tx_time.get(0);   // value, not pointer
    steep_mgr_.on_tx(
        reinterpret_cast<srsran::steep_manager::cf_t*>(buffer.get(0)),
        nof_samples,
        ts.full_secs,
